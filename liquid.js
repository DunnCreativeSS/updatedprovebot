...
if (process.env.binApiSecret == undefined){
  apisecret = "WZdACOhcLSs04m5p2MUhGgG2NC9qA6Crh11ptlTE0E8E6uyWs1qpTif0YI5DjeiuAvnT6ozjvuShTecE1w/KFw=="
}   else {
  apisecret = process.env.binApiSecret
}

let liquid  = new ccxt.liquid ({ 'enableRateLimit': true, apiKey: apikey, secret: apisecret })



setInterval(async function(){
  let tickers = await liquid.fetchTickers()
  for (var t in tickers) {
    
    if (modular.asks[tickers[t].symbol] == undefined) {
            modular.asks[tickers[t].symbol.replace('/','')] = {}
            modular.bids[tickers[t].symbol.replace('/','')] = {}
        ...
        modular.asks[tickers[t].symbol.replace('/','')]['default'] = tickers[t].ask
        modular.bids[tickers[t].symbol.replace('/','')]['default'] = tickers[t].bid
...
async function getthebals(){
  btcs138 = {}
  let bals = await liquid.fetchBalance();
  myBals = bals
...
if ((modular.asks[myBals[bal].asset + '/' + modular.bases[b]]) != undefined){
 ...
 if (modular.bases[b] == 'BTC'){
       ...
       if (myBals[bal].asset == 'NEO'){
       console.log(myBals[bal].asset)
...
module.exports.exchangeCandlesAndBooks = async function exchangeCandlesAndBooks(tickVols){
        ...
        if(liquid.has['fetchOHLCV'] == 'emulated'){
          for (var t in tickVols){
          
      let since = liquid.milliseconds () - 86400000 / 2
    let limit = 24;
          ...
          let sleep = (ms) => new Promise (resolve => setTimeout (resolve, ms));
                  ...
                  await sleep (liquid.rateLimit) // milliseconds

        let candles2 =  (await liquid.fetchOHLCV (t, '1h', since, limit)) // one minute
...
async function setBal(){
  ...
  let balances =  await liquid.fetchBalance();
  let bals138 = {}
...
module.exports.exchangeDoTrades = async function exchangeDoTrades(symbol){
  let since = liquid.milliseconds () - 86400000
    let limit = 100;
 let trades = await liquid.fetchMyTrades (symbol, since, limit)
...
module.exports.exchangeInfo = async function exchangeInfo(){
  console.log('exinfo')
  let exchange = (await liquid.fetchMarkets())
    for (var ex in exchange) {
      ...
      if (symbol == 'HOT/ETH'){
        ...
        filters[symbol] = {
            ...
            'minQty': parseFloat(1),
            'tickSize': (parseFloat(exchange[ex].precision.price)),
            'stepSize': (parseFloat(exchange[ex].precision.amount)),
            'minNotional': parseFloat(0.000001)
        }
        symbol = exchange[ex].symbol.replace('/','')
        filters[symbol] = {
            'minPrice': parseFloat(0),
            'minQty': parseFloat(1),
            'tickSize': (parseFloat(exchange[ex].precision.price)),
...
module.exports.exchangeOrder = async function exchangeOrder(symbol, side, qty, price, type){
  ...
  console.log(price)
  console.log(type)
  let o = await liquid.createOrder (symbol, type.toLowerCase(), side.toLowerCase(), qty, price)
  console.log(o)
  return(o)
}