...
setInterval(async function(){
  let tickers = await pClient.swap().getInstruments()
  ...
  for (var t in tickers){
    let ticker = await pClient.swap().getTicker(tickers[t].instrument_id)
    if (true){
    if (modular.asks[tickers[t].instrument_id] == undefined) {
        ...
        modular.asks[tickers[t].instrument_id]['default'] = ticker.last
        modular.bids[tickers[t].instrument_id]['default'] = ticker.last
        modular.asks[tickers[t].underlying_index]['default'] = ticker.last
        modular.bids[tickers[t].underlying_index]['default'] = ticker.last
...
////console.log(asset)
        ...
        let book = await  pClient.swap().getDepth(tickers[t].instrument_id, 10)
...
async function getthebals(){
  ...
  for (var p in modular.pairs){
    let balance= await authClient.swap().getAccount(modular.pairs[p]);
    balances[modular.pairs[p].substring(0,3).toLowerCase()] = balance
  }
     let total2 = 0
     bals = {}
     for (var b in balances){
       bals[b] = parseFloat(balances[b].info.total_avail_balance)
       
       total2 += ((parseFloat(balances[b].info.total_avail_balance) * parseFloat(modular.bids[b.toUpperCase()]['default']) / parseFloat(modular.bids['BTC-USD-SWAP']['default'])))
     }
...
async function test(){
  ...
  let os = await authClient.swap().getOrders('ETC-USD-190628', {status:"0"})
  console.log(os)
  os = os.order_info
  for (var o in os){
...
module.exports.exchangeOpenOrders = async function exchangeOpenOrders(symbol){
  ...
  let os = await authClient.swap().getOrders(symbol, {status:"0"})
  os = os.order_info
  for (var o in os){
    os[o].symbol = os[o].instrument_id
    if (os[o].type =='1'){
...
cancelled.push(order.order_id)
 console.log(await privateClient.cancelOrder(console.log, order.instrument_id, order.order_id));
}
}
...
module.exports.exchangeCandlesAndBooks = async function exchangeCandlesAndBooks(tickVols){
        ...
        let candles2 =  (await pClient.swap().getCandles(tickVols)) // one minute
...
module.exports.exchangeDoTrades = async function exchangeDoTrades(symbol){
    ...
    let orders = await authClient.swap().getOrders({instrument_id: symbol, status: 'filled'})
    for (var o in orders){
 let trades = await authClient.swap().getFills({instrument_id: symbol, order_id: orders[o].order_id})
...
module.exports.exchangeOrder = async function exchangeOrder(symbol, side, qty, price, type){
  ...
  let o = await authClient.swap().postOrder({"price":price.toString(),"type":which, "instrument_id":symbol, "size":qty.toString(), "leverage":"10"});
  console.log(o)
  return(o)
}